...
export default function EditRoutinePage({
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          const r = data as Routine & { routine_exercises: (RoutineExercise & { exercise: Exercise })[] };
          setName(r.name);
          setExercises(
            (r.routine_exercises || [])
  ...
  async function saveRoutine() {
    ...
    // Delete all existing routine exercises and re-insert
    ...
    <div className="px-4 safe-top">
      <div className="flex items-center justify-between pt-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-white">Edit Routine</h1>
        </div>
        <button
          onClick={deleteRoutine}
          disabled={deleting}
          className="text-danger text-sm font-semibold active:opacity-60"
        >
          Delete
        </button>
      </div>
        ...
        <div>
          ...
          {exercises.length === 0 ? (
            <Card className="text-center py-8">
              <p className="text-text-secondary">No exercises added</p>
            </Card>
          ) : (
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="w-14 bg-card-elevated text-white text-center rounded-ios px-2 py-1.5 text-sm focus:outline-none"
                        min="1"
                        max="20"
                      />
                    </div>
